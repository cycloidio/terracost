...
func main() {
	...
	flag.StringVar(&flagestimateHCLProvider, "estimate-hcl-provider", flagestimateHCLProvider, "Terraform provider used [aws|azurerm]")
	...
	if flagIngest {
		...
		if flagestimateHCLProvider == "azurerm" {
			ingestRegion = flagAzureIngestRegion
		}
		ingest(flagestimateHCLProvider, ingestRegion, backend, db)
	}
...
func ingest(flagestimateHCLProvider string, region string, backend *mysql.Backend, db *sql.DB) {
	fmt.Printf("Ingestion %s\n", flagestimateHCLProvider)
	...
	if flagestimateHCLProvider == "aws" {
	...
	} else if flagestimateHCLProvider == "azurerm" {
